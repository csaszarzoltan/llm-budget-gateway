...
const contextKey='gateway-console-context',recentKey='gateway-console-recents',favoriteKey='gateway-console-favorites',progressKey='gateway-console-workflow-progress',telemetryKey='gateway-console-usage-counters';
...
function trackUsage(event){{const allowed=new Set(['workflow_started','workflow_resumed','workflow_step_completed','workflow_completed','request_succeeded','request_failed']);if(!allowed.has(event))return;const counters=storedJSON(telemetryKey,'{{}}');counters[event]=(Number(counters[event])||0)+1;localStorage.setItem(telemetryKey,JSON.stringify(counters))}}
function workflowProgress(){{return storedJSON(progressKey,'{{}}')}}
function saveWorkflowProgress(){{if(!activeWorkflow)return;const all=workflowProgress();const entry=all[activeWorkflow.id]||{{completedSteps:[],currentIndex:0}};entry.currentIndex=activeWorkflowIndex;entry.completedSteps=[...new Set(entry.completedSteps)].filter(i=>Number.isInteger(i)&&i>=0&&i<activeWorkflow.steps.length);all[activeWorkflow.id]=entry;localStorage.setItem(progressKey,JSON.stringify(all));restoreWorkflowProgress()}}
function restoreWorkflowProgress(){{const all=workflowProgress();$$('[data-workflow-progress]').forEach(label=>{{const workflow=workflows.find(x=>x.id===label.dataset.workflowProgress),entry=all[label.dataset.workflowProgress];if(!workflow||!entry){{label.textContent='Not started';return}}const done=(entry.completedSteps||[]).length;label.textContent=done===workflow.steps.length?'Completed':`${{done}} of ${{workflow.steps.length}} complete`;const start=$(`[data-workflow-start="${{workflow.id}}"]`);if(start)start.textContent=done?'Resume':'Start'}})}}
function completeCurrentStep(){{if(!activeWorkflow)return;const all=workflowProgress(),entry=all[activeWorkflow.id]||{{completedSteps:[],currentIndex:activeWorkflowIndex}};if(!entry.completedSteps.includes(activeWorkflowIndex)){{entry.completedSteps.push(activeWorkflowIndex);trackUsage('workflow_step_completed')}}entry.currentIndex=activeWorkflowIndex;all[activeWorkflow.id]=entry;localStorage.setItem(progressKey,JSON.stringify(all));if(entry.completedSteps.length===activeWorkflow.steps.length){{trackUsage('workflow_completed');toast('Workflow completed')}}else toast('Step marked complete');openWorkflowStep(activeWorkflowIndex)}}
function resetWorkflowProgress(){{if(!activeWorkflow)return;const all=workflowProgress();delete all[activeWorkflow.id];localStorage.setItem(progressKey,JSON.stringify(all));activeWorkflowIndex=0;restoreWorkflowProgress();openWorkflowStep(0);toast('Workflow progress reset')}}
function resumeWorkflow(id){{activeWorkflow=workflows.find(x=>x.id===id);if(!activeWorkflow)return;const entry=workflowProgress()[id],index=entry?Math.min(Number(entry.currentIndex)||0,activeWorkflow.steps.length-1):0;trackUsage(entry?'workflow_resumed':'workflow_started');$('#workflowGuide').classList.add('open');$('#workflowGuideTitle').textContent=activeWorkflow.title;openRunner(activeWorkflow.steps[index][0],activeWorkflow.steps[index][1]);openWorkflowStep(index)}}
...
function openWorkflowStep(index){{if(!activeWorkflow)return;activeWorkflowIndex=Math.max(0,Math.min(index,activeWorkflow.steps.length-1));const step=activeWorkflow.steps[activeWorkflowIndex],entry=workflowProgress()[activeWorkflow.id]||{{completedSteps:[]}};updateRunner(step[0],step[1]);applyStepPreset(step);$('#workflowProgress').textContent=`Step ${{activeWorkflowIndex+1}} of ${{activeWorkflow.steps.length}} · ${{entry.completedSteps.length}} complete`;$('#workflowSteps').innerHTML=activeWorkflow.steps.map((x,i)=>`<div class='workflow-step ${{entry.completedSteps.includes(i)?'completed ':''}}${{i===activeWorkflowIndex?'current':''}}'><button type='button' data-step-index='${{i}}' ${{i===activeWorkflowIndex?'aria-current="step"':''}}><strong>${{entry.completedSteps.includes(i)?'✓ ':''}}${{i+1}}. ${{centers[x[0]].name}}</strong><br><small>${{x[1]}}</small></button></div>`).join('');$$('[data-step-index]').forEach(button=>button.onclick=()=>openWorkflowStep(Number(button.dataset.stepIndex)));$('#workflowPrevious').disabled=activeWorkflowIndex===0;$('#workflowNext').disabled=activeWorkflowIndex===activeWorkflow.steps.length-1;$('#workflowComplete').disabled=entry.completedSteps.includes(activeWorkflowIndex);addRecent(step[0],step[1]);saveWorkflowProgress();rememberContext()}}
function startWorkflow(id){{resumeWorkflow(id)}}
...
function actionableFailure(status){{if(status===401)return'Check the bearer key and service-specific authentication.';if(status===404)return'Check the selected capability or custom path.';if(status===412)return'The hard budget was exceeded. Review the applicable scope before retrying.';if(status===422)return'Review required fields and value ranges in the request body.';if(status===429)return'Rate or token capacity was exceeded. Use the 412 / 429 / 502 recovery workflow.';if(status>=500)return'Check service health, logs, and the recovery workflow before retrying.';return'Review the response and API documentation.'}}
$('#sendRequest').onclick=async()=>{{const sendRequest=$('#sendRequest'),method=$('#runnerMethod').value,url=endpoint(),opts={{method,headers:headers(),signal:AbortSignal.timeout(30000)}};const parsed=validateRequestBody();if(!['GET','HEAD'].includes(method)&&parsed===null)return;rememberContext();sessionStorage.setItem('key:'+$('#runnerCenter').value,$('#runnerKey').value);sendRequest.disabled=true;sendRequest.setAttribute('aria-busy','true');try{{if(!['GET','HEAD'].includes(method))opts.body=JSON.stringify(parsed);$('#result').textContent='Sending '+method+' '+url+' …';const r=await fetch(url,opts);const text=await r.text();let body=text;try{{body=JSON.stringify(JSON.parse(text),null,2)}}catch{{}}$('#result').textContent=`HTTP ${{r.status}} ${{r.statusText}}\n\n${{body}}${{r.ok?'':'\n\nNext action: '+actionableFailure(r.status)}}`;trackUsage(r.ok?'request_succeeded':'request_failed');if(r.ok&&activeWorkflow)completeCurrentStep()}}catch(e){{trackUsage('request_failed');$('#result').textContent='Request timed out or the service is unreachable\n\n'+e.message+'\n\nCheck service health, CORS, and the selected base URL.'}}finally{{sendRequest.disabled=false;sendRequest.setAttribute('aria-busy','false')}}}};
