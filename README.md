...
![Version 10.1.0](https://img.shields.io/badge/version-9.4.0-blue.svg)
...
The package version is `10.1.0`. Historical center versions in documentation describe the release in which a capability family was introduced. They are all included in the current package.
...
Version 10.1 implements the three P0 priorities validated in `research-findings.md`: one live graphical cockpit, a pre-step Agent Runaway Firewall, and schema-generated guided forms. These address fragmented operations, runaway agent spend, and raw-JSON onboarding friction.
...
Version 10.1 continues the priority roadmap from `research-findings.md` with the first two P1 differentiators:
...
Version 10.1 implements the remaining high-value research recommendation for signed, reproducible distribution:
...
Version 10.1 completes the feature roadmap prioritized in `research-findings.md`. In addition to the cockpit, runaway firewall, generated forms, traces, outcome economics, and verified supply chain, this release adds:
...
- **Tenant-isolated repository:** canonical per-tenant exports enable migration from local SQLite to a shared production repository.
- **Transactional shared counters:** idempotent hard-limit reservations support multi-process deployments without concurrent overshoot.
- **Signed SSO identity:** trusted reverse proxies can pass canonical HMAC-signed claims with tenant, expiry, and role enforcement.
- **Interactive trace preview:** the React cockpit visualizes agent, model, and tool relationships and provides a live simulator flow.
