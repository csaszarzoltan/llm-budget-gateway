...
function openRunner(slug='gateway',cap=''){{updateRunner(slug,cap);$('#drawer').classList.add('open');$('#drawer').setAttribute('aria-hidden','false');setTimeout(()=>$('#runnerCapability').focus(),50)}}
function closeRunner(){{$('#drawer').classList.remove('open');$('#drawer').setAttribute('aria-hidden','true')}}
$('#runnerCenter').onchange=e=>updateRunner(e.target.value,'');$('#drawerClose').onclick=closeRunner;$('#openRunner').onclick=()=>openRunner();$('#heroRunner').onclick=()=>openRunner();$$('[data-action=run]').forEach(b=>b.onclick=()=>openRunner(b.dataset.center,b.dataset.capability));$$('[data-action=open-center]').forEach(b=>b.onclick=()=>{{const c=centers[b.dataset.center];window.open(`http://localhost:${{c.port}}${{c.dashboard}}`,'_blank')}});
...
$('#sendRequest').onclick=async()=>{{const method=$('#runnerMethod').value,url=endpoint(),opts={{method,headers:headers()}};sessionStorage.setItem('key:'+$('#runnerCenter').value,$('#runnerKey').value);try{{if(!['GET','HEAD'].includes(method))opts.body=JSON.stringify(JSON.parse($('#runnerBody').value||'{{}}'));$('#result').textContent='Sending '+method+' '+url+' …';const r=await fetch(url,opts);const text=await r.text();let body=text;try{{body=JSON.stringify(JSON.parse(text),null,2)}}catch{{}}$('#result').textContent=`HTTP ${{r.status}} ${{r.statusText}}\n\n${{body}}`}}catch(e){{$('#result').textContent='Request failed\n\n'+e.message+'\n\nCheck that the service is running and permits this console origin.'}}}};
