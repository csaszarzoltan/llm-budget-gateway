...
def create_control_app(path: str = "control-plane.db") -> FastAPI:
    cp=ControlPlane(path); app=FastAPI(title='LLM Budget Gateway Control API',version='1.0')
    def ctx(x_tenant_id:str|None,x_role:str|None):
        if not x_tenant_id or not x_role: raise HTTPException(401,'X-Tenant-Id and X-Role are required')
        return x_tenant_id,x_role
    @app.exception_handler(PermissionDenied)
    async def permission(_r:Request,e:PermissionDenied): return PlainTextResponse(str(e),status_code=403)
    @app.get('/control',response_class=HTMLResponse)
    async def control(): return UI
    @app.get('/v1/admin/dashboard')
    async def dashboard(x_tenant_id:str|None=Header(None),x_role:str|None=Header(None)):
        t,r=ctx(x_tenant_id,x_role);return cp.dashboard(t,r)
    @app.post('/v1/admin/workspace')
    async def workspace(body:dict,x_tenant_id:str|None=Header(None),x_role:str|None=Header(None)):
        t,r=ctx(x_tenant_id,x_role);cp.configure_workspace(t,r,str(body.get('name','')).strip());return {'status':'active'}
    @app.post('/v1/admin/keys',status_code=201)
    async def keys(body:dict,idempotency_key:str|None=Header(None),x_tenant_id:str|None=Header(None),x_role:str|None=Header(None)):
        t,r=ctx(x_tenant_id,x_role);return cp.issue_key(t,r,body.get('label',''),body.get('models',[]),body.get('expires_at'),idempotency_key)
    @app.get('/v1/admin/keys')
    async def list_keys(x_tenant_id:str|None=Header(None),x_role:str|None=Header(None)):
        t,r=ctx(x_tenant_id,x_role);return {'items':cp.list_keys(t,r)}
    @app.put('/v1/admin/budgets/{scope}')
    async def budget(scope:str,body:dict,x_tenant_id:str|None=Header(None),x_role:str|None=Header(None)):
        t,r=ctx(x_tenant_id,x_role);cp.set_budget(t,r,scope,float(body['limit']));return cp.budget_status(t,scope)
    @app.get('/v1/admin/spend.csv',response_class=PlainTextResponse)
    async def spend(x_tenant_id:str|None=Header(None),x_role:str|None=Header(None)):
        t,r=ctx(x_tenant_id,x_role);return cp.export_spend_csv(t,r)
    app.state.control_plane=cp; return app
