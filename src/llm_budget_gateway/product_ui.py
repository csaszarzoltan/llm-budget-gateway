...
'keys':('Key access console','Issue, rotate and revoke credentials','Create key'),
...
'activity':('Operations center','Resolve warnings and failed operations','Review queue')}

def setup_progress(state:Mapping[str,int])->dict[str,object]:
 steps=('workspace','key','budget','route'); done=sum(bool(state.get(x)) for x in steps)
 return {'completed':done,'total':4,'next':next((x for x in steps if not state.get(x)),None)}
def spend_view(records:Sequence[Mapping[str,object]],model:str|None=None,budget:float=0)->dict[str,object]:
 rows=[r for r in records if model is None or r.get('model')==model];total=sum(float(r.get('cost',0)) for r in rows)
...
def activity_view(items:Sequence[Mapping[str,object]])->dict[str,int]:
 return {'actionable':sum(x.get('state') in {'warning','failed'} for x in items),'total':len(items)}
def render_page(page:str,context:Mapping[str,object])->str:
 if page not in PAGES:raise KeyError(page)
 title,subtitle,action=PAGES[page];role=str(context.get('role','viewer'));tenant=escape(str(context.get('tenant','Workspace')))
 if page=='keys' and role not in {'admin','security'}:action=''
 nav=''.join(f'<a href="/control/{p}" aria-current="{ "page" if p==page else "false"}">{n[0]}</a>' for p,n in PAGES.items())
 cards=''.join(f'<article class="card"><h2>{x}</h2><p>Ready for tenant-scoped data.</p><button>View details</button></article>' for x in ('Overview','Current state','Next safe action'))
