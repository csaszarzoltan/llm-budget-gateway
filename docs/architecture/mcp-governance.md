...
**Assurance Center integration (wired):** `assurance_api.py`'s `services`
dict registers a `"mcp-governance"` capability —
`lambda b: MCPGovernanceReport().assess(b)` — so a request to the assurance
path `.../v1/assurance/mcp-governance` returns the posture assessment
(`{risk_tier, gaps, recommendation}`), and the Assurance Center dashboard
surfaces it as a new capability card ("MCP governance"). Note that
`assess` takes the report dict as a single Mapping argument (unlike the
other services' `**b` expansion), so the lambda passes `b` directly. The
standalone `GET /v1/mcp/report` endpoint on the governance app returns the
same posture snapshot. No change to the assurance suite's other 20
capabilities.
